 ...
 *
 * $Id: remote.cc,v 1.17 2008/06/12 20:21:44 joerg_wunsch Exp $
 */
...
const char *GdbRemote::GDB_CAUSE = "gdb communications failure";
void GdbRemote::check(int status) {
	unixCheck(status, GDB_CAUSE);
}

void GdbRemote::setFile(int fd)
{
...
void GdbRemote::waitForOutput(void)
{
	FDSelect fds;
...
void GdbRemote::putDebugChar(char c)
{
...
void GdbRemote::waitForInput(void)
{
	FDSelect fds;
	fds.add(gdbFileDescriptor);
...
int GdbRemote::getDebugChar(void)
{
	uint8_t c = 0;
	int result;
...
int GdbRemote::checkForDebugChar(void)
{
	uint8_t c = 0;
	int result;
...
void GdbRemote::vgdbOut(const char *fmt, va_list args)
{
...
void GdbRemote::gdbOut(const char *fmt, ...)
{
  va_list args;
  va_start(args, fmt);
  vgdbOut(fmt, args);
...
void GdbRemote::reportStatusExtended(int sigval)
{
	uint8_t *jtagBuffer;
...
/** Fill 'remcomOutBuffer' with a status report for signal 'sigval' **/
void GdbRemote::reportStatus(int sigval)
{
...
unsigned int GdbRemote::readLWord(unsigned int address)
{
...
unsigned int GdbRemote::readBWord(unsigned int address)
{
	unsigned char *mem = theJtagICE->jtagRead(DATA_SPACE_ADDR_OFFSET + address, 2);
...
unsigned int GdbRemote::readSP(void)
{
	return readLWord(0x5d);
}

bool GdbRemote::handleInterrupt(void)
{
	bool result;
...
void GdbRemote::putpacket(char *buffer)
{
	unsigned char checksum;
	int count;
	char ch;
...
void GdbRemote::repStatus(bool breaktime)
{
	if (breaktime)
...
void GdbRemote::talkToGdb(void)
{
    ...
    case 'M':
			{
				uint8_t *jtagBuffer;
        int lead = 0;        static bool last_orphan_pending = false;
